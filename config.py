...
# 监测类型：news=新闻爬虫，meeting=会议/论坛监测
MONITOR_TYPE_NEWS = "news"
MONITOR_TYPE_MEETING = "meeting"
VALID_MONITOR_TYPES = (MONITOR_TYPE_NEWS, MONITOR_TYPE_MEETING)
...
class MeetingConfig:
    ...
    # 百度等搜索引擎：多关键词空格合并一次检索
    SEARCH_ENGINE_COMBINED_KEYWORDS: bool = _env_bool(
    ...
    SEARCH_ENGINE_EVENT_KEYWORDS: list[str] = [
        kw.strip()
        for kw in _env(
            "MEETING_SEARCH_EVENT_KEYWORDS",
            "论坛,会议,研讨会,峰会,博览会,高峰论坛,产业大会,学术会议",
        ).split(",")
        if kw.strip()
    ]
...
CREATE TABLE IF NOT EXISTS crawl_sites (
    ...
    enable_news_crawl      BOOLEAN NOT NULL DEFAULT TRUE,
    enable_meeting_monitor BOOLEAN NOT NULL DEFAULT FALSE,
...
COMMENT ON COLUMN crawl_sites.is_active IS '站点总开关：FALSE=新闻与会议监测均不执行';
COMMENT ON COLUMN crawl_sites.enable_news_crawl IS '是否参与新闻爬虫';
COMMENT ON COLUMN crawl_sites.enable_meeting_monitor IS '是否参与会议/论坛监测';
...
CREATE TABLE IF NOT EXISTS crawl_keywords (
    ...
    monitor_type    VARCHAR(20) NOT NULL DEFAULT 'news',
    is_active       BOOLEAN DEFAULT TRUE,
...
COMMENT ON COLUMN crawl_keywords.keyword IS '关键词内容';
COMMENT ON COLUMN crawl_keywords.keyword_type IS '关键词分类（如：创新平台、实验室、成果转化）';
COMMENT ON COLUMN crawl_keywords.monitor_type IS '用途：news=新闻爬虫，meeting=会议/论坛监测';
...
COMMENT ON TABLE  meeting_items IS '会议/论坛监测结果，仅存 AI 判定通过的条目';
COMMENT ON COLUMN meeting_items.url IS '原文链接，唯一索引用于去重';
COMMENT ON COLUMN meeting_items.summary IS '列表摘要或 AI 改写';
COMMENT ON COLUMN meeting_items.event_type IS '会议/论坛/研讨会/峰会等';

CREATE INDEX IF NOT EXISTS idx_meeting_items_first_seen ON meeting_items(first_seen_at DESC);
"""
